...
public class RopeScriptEdit : MonoBehaviour {
	/*========================================
	==  Physics Based Rope				==
	==  File: Rope.js					  ==
	...
	*/
	...
	void LateUpdate()
	{
		...
		if(rope) {
			for(int i=0;i<joints.Length;i++) {
				if(i == 0) {
					joints[i].transform.position = start.position;
					line.SetPosition(i,start.position);
				} else
				if(i == joints.Length - 1) {
					joints[i].transform.position = target.position;
	...
	void BuildRope()
	{
		...
		segments = (int)(Vector3.Distance(start.position,target.position)*resolution);
		line.positionCount = segments;
		segmentPos = new Vector3[segments];
		joints = new GameObject[segments];
		segmentPos[0] = new Vector3(start.position.x, start.position.y, start.position.z);
		//segmentPos[segments-1] = target.position;
 
		// Find the distance between each segment
		int segs = segments-1;
		seperation = ((target.position - start.position) / segs);
 
		for(int s=0;s <= segments-1;s++)
		{
			// Find the each segments position using the slope from above
			Vector3 vector = (seperation * s) + start.position;	
			segmentPos[s] = vector;
		...
		/*int lastSegmentIndex = segments-1;

		joints[lastSegmentIndex] = new GameObject("Last Joint_" + (lastSegmentIndex));
		

		Rigidbody2D rigid = joints[lastSegmentIndex].AddComponent<Rigidbody2D>();
		DistanceJoint2D distJoint = joints[lastSegmentIndex].gameObject.AddComponent<DistanceJoint2D>();

		distJoint.autoConfigureConnectedAnchor = false;
		distJoint.autoConfigureDistance = false;

		distJoint.distance = seperation.magnitude;

		rigid.drag = ropeDrag;
		rigid.mass = ropeMass;

		joints[lastSegmentIndex].transform.position = target.position;
		//distJoint.autoConfigureConnectedAnchor = true;
		distJoint.connectedBody = joints[lastSegmentIndex-1].transform.GetComponent<Rigidbody2D>();
		*/
	...
	void AddJointPhysics(int n)
	{
		...
		SpringJoint2D distJoint = joints[n].AddComponent<SpringJoint2D>();
		...
		distJoint.frequency = 10f;
		distJoint.dampingRatio = 1f;

		//charJoint.swingAxis = swingAxis;
		//SoftJointLimit limit_setter = charJoint.lowTwistLimit;
		//limit_setter.limit = lowTwistLimit;
		//charJoint.lowTwistLimit = limit_setter;
		//limit_setter = charJoint.highTwistLimit;
		//limit_setter.limit = highTwistLimit;
		//charJoint.highTwistLimit = limit_setter;
		//limit_setter = charJoint.swing1Limit;
		//limit_setter.limit = swing1Limit;
		//charJoint.swing1Limit = limit_setter;
		...
		//distJoint.autoConfigureConnectedAnchor = true;
		if(n==0){	
			joints[n].transform.position = start.position;
			//distJoint.connectedBody = start.GetComponent<Rigidbody2D>();
		} else 
		if(n==segments-1){
			joints[n].transform.position = target.position;
			distJoint.connectedBody = joints[n-1].GetComponent<Rigidbody2D>();	
			//DistanceJoint2D distJoint2 = joints[n].gameObject.AddComponent<DistanceJoint2D>();
			//distJoint2.autoConfigureDistance = false;
			//distJoint2.connectedBody = target.GetComponent<Rigidbody2D>();
			//distJoint2.distance = 0f;
		} else
		{
